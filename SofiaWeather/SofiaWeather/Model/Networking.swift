...
final class Networking {
    ...
    typealias ForecastCompletion = (Weather.Forecast?) -> ()
    ...
    static func getForecast(completion: @escaping ForecastCompletion) {
        let url = URL(string: "https://api.openweathermap.org/data/2.5/forecast?q=Sofia,bg&units=metric&lang=bg&appid=7df485076cd86fd0dbab7e38b388e8dc")!
        ...
        URLSession.shared.dataTask(with: url) { (data, _, error) in
            ...
            do {
                let forecast = try JSONDecoder().decode(Weather.Forecast.self, from: data)
